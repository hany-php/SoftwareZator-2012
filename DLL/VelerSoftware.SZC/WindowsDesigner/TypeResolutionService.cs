	...
	public class TypeResolutionService : ITypeResolutionService
	{
        ...
        public Type GetType(string name, bool throwOnError, bool ignoreCase)
        {
            if (name.Trim() != null)
			{
                ...
                // Fallback: Try to load Krypton Toolkit if not found
                if (!ok && name.StartsWith("ComponentFactory.Krypton"))
                {
                    try
                    {
                        string appDir = AppDomain.CurrentDomain.BaseDirectory;
                        string kryptonPath = Path.Combine(appDir, "ComponentFactory.Krypton.Toolkit.dll");
                        
                        // Try looking in AUTRE DLL relative to default project structure if not in execution dir
                        if (!File.Exists(kryptonPath))
                        {
                             string rootPath = Path.GetFullPath(Path.Combine(appDir, "..\\.."));
                             string autrePath = Path.Combine(rootPath, "AUTRE DLL\\ComponentFactory.Krypton.Toolkit.dll");
                             if (File.Exists(autrePath)) kryptonPath = autrePath;
                        }

                        if (File.Exists(kryptonPath))
                        {
                            Assembly kryptonAssembly = Assembly.LoadFrom(kryptonPath);
                            Type t = kryptonAssembly.GetType(name, false, ignoreCase);
                            if (t != null) return t;
                        }
                    }
                    catch { }
